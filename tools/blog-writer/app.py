...
def extract_legal_claims(body_html: str) -> list[str]:
    ...
    resp = client.messages.create(
        ...
        max_tokens=8000,
        system=(
            "너는 여행 블로그 본문에서 '법적 강제력이 있는' 규정 문장만 "
            "추출하는 도구다. 문화·예절·종교 관습·사기 수법·안전 조언은 "
            "절대 추출하지 마라. 반드시 submit_legal_claims 도구를 호출해 "
            "결과를 제출하라."
        ),
        messages=[{
            "role": "user",
            "content": (
                "아래 HTML 본문에서 다음 조건을 모두 만족하는 문장만 추출하라.\n"
                "\n"
                "포함 대상 (법률·처벌·규제 중 '법적 강제력이 있는 것'만):\n"
                "- 법 조항 (조 번호, 법률·조약 명칭이 있는 규정)\n"
                "- 형량 (징역, 금고, 구금 등 기간이 언급된 처벌)\n"
                "- 벌금·과태료 (금액이 언급된 처벌)\n"
                "- 반입/반출 금지 (관세·검역법 등 법으로 금지된 물품)\n"
                "- 비자·체류 관련 법정 규정 (기간 초과 처벌, 무비자 기간 등)\n"
                "- 복장·촬영·행위 중 위반 시 법적 처벌 대상이 되는 것\n"
                "\n"
                ...
                "\n"
                "'※ 확인 필요' 표시 처리:\n"
                "- '※ 확인 필요' 라는 문구 자체는 주장이 아니므로 그것만 단독으로 "
                "  추출하지 마라.\n"
                "- 위 '포함 대상'에 해당하는 법률 주장 문장에 '※ 확인 필요'가 "
                "  붙어 있다면, 그 문장을 추출하되 문장 끝에 '(※ 확인 필요)' 를 "
                "  붙여서 우선 검증 대상임을 표시하라.\n"
                "\n"
                "규칙:\n"
                ...
                "- 법적 강제력이 있는 조항이 하나도 없으면 빈 배열을 반환하라.\n\n"
                f"본문:\n{body_html}"
            ),
        }],
        tools=[CLAIMS_TOOL],
